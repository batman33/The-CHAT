    ...
    <>
      <Combobox value={selected} onChange={onSelected} multiple>
        <div className="relative mt-1">
          <div className="relative w-full cursor-default overflow-hidden rounded-lg text-left shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
            ...
            />
            <Combobox.Button className="absolute inset-y-0 right-0 flex items-center pt-2">
              <ChevronUpDownIcon
                ...
                <div className="relative cursor-default select-none px-4 py-2 text-gray-700">
                 ...
                 filteredPeople.map((person) => (
                   <Combobox.Option key={person.id}
                                    value={person}
                                    className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-teal-600 text-white'
      ...
      {selected && (
        <div className="flex gap-2 mt-3 flex-wrap">
          {selected.map((person) => (
            <div className="badge badge-primary gap-2 text-nowrap" key={person.id}>
              {person.name}
            </div>
          ))}
        </div>
